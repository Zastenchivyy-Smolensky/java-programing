...
public class rp0401 extends NewtonMethod
{
	double f(double x) {//f(x)=0となる解を求める
		return x*x*x*x-3.1*x*x*x+2.1*x*x+0.01;//抽象メソッドをオーバーライド
		
	}
	double df(double x) {// f'(x) ※f(x)の微分
		return 4*x*x*x-9.3*x*x+4.2*x;//抽象メソッドをオーバーライド
	}
	void result() {
		if(status==0) {
			System.out.println("方程式:x="+newx+"です");//結果を表示
		}
	}
	public static void main(String args[]) {
		...
		System.out.println("方程式: x^4−3.1x^3+2.1x^2+0.01");//式を表示
		
		double x0 = -0.5;//1つ目の解を求める
		System.out.println("¥n 初期値x0="+x0+"で解を求める");
		nw.solve(x0);//初期値x0=-0.5で方程式をとく
		nw.result();//結果表示
		
		//2つ目の解を求める
		x0=0.8;
		System.out.println("¥n初期値 x0="+x0+"で解を求める");
		nw.solve(x0);//初期値x0=0.8で方程式をとく
		nw.result();//結果表示
		
		//3つ目の解を求める
		x0=1.0;
		System.out.println("¥n初期値 x0="+x0+"で解を求める");
		nw.solve(x0);//初期値x0=1.0で方程式をとく
		nw.result();//結果表示
		
		//4つ目の解を求める
		x0=5.0;
		System.out.println("¥n初期値 x0="+x0+"で解を求める");
		nw.solve(x0);//初期値x0=5.0で方程式をとく
		nw.result();//結果表示
	}
